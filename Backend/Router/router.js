...
const {
  upsertInventory,
} = require("../model/Inventory/upsertInventorymanagement");
...
const {
  deleteInventory,
} = require("../model/Inventory/inventoryMangementDelete");
...
const {
  dataloaderEnquiry,
} = require("../model/Enquiry/dataloaderEnquiryinsert");
const { dataloaderAccount } = require("../model/Account/dataloaderaccount");
const { dataloaderDeal } = require("../model/Deal/dataloaderDeal");
const {
  upsertOpportunityInventory,
} = require("../model/opportunity_inventory/upsertoppinv");
const {
  getOpportunityInventory,
} = require("../model/opportunity_inventory/getoppinv");
const {
  deleteOpportunityInventory,
} = require("../model/opportunity_inventory/deleteoppinv.js");
const csvtojson = require("csvtojson");
const { authVerify } = require("../helpers/authverify");
const {
  Accouninsertschema,
  dataloaderAccountinsertschema,
} = require("../model/schema/accountSchema");
...
function getdatafromreact(fastify, options, done) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        console.log(
          "data loader Deal  data  " + JSON.stringify(request.files[0].filename)
        );
        const files = request.files[0].filename;
        const csvfilepath = "uploads/" + files;
        await csvtojson()
  ...
  fastify.post(
    "/api/Dashboard",
    /*Accouninsertschema,*/ async (request, reply) => {
      console.log("upsert route called");
      console.log("request body " + request.body);
      try {
        console.log("upsert account try ");
  ...
  fastify.post(
    "/api/account",
    /*Accouninsertschema,(validation)*/ async (request, reply) => {
  ...
  fastify.post("/api/user", async (request, reply) => {
    ...
    try {
      ...
      if (result) {
        ...
        reply.send({
          status: "failure",
          content: "No Data Inserted or updated",
        });
      }
    } catch (e) {
  ...
  fastify.post("/api/event", async (request, reply) => {
    console.log(
      "upsert task -event route called " + JSON.stringify(request.body)
    );
    try {
  ...
  fastify.get(
    "/api/accounts",
    /*{preHandler:authVerify},*/ async (request, reply) => {
      try {
  ...
  fastify.get("/api/inventory/related/account/:id", async (request, reply) => {
    console.log(
      "Inside  get Inventories by acc id  Router " + request.params.id
    );
    try {
  ...
  fastify.post("/api/getInventoriesbyOppid", async (request, reply) => {
    ...
    console.log(
      "Inside  get Inventories by opp id  Router " + request.query.searchId
    );
    try {
